...
def corrupt_crop(images):
    ...
    for img in images:
        ...
        corrupted_img = [[pixel if x <= i < x + 10 and y <= j < y + 10 else 0 for j, pixel in enumerate(row)]
...
def main():
    ...
    convergence_rates_flip_sync = []
    convergence_rates_flip_async = []
    ...
    for prob in probabilities:
        successful_convergences_sync = 0
        successful_convergences_async = 0
        steps_data = []
        run = 0
        ...
        for _ in range(20):
            ...
            for img, original in zip(corrupted_images, dataset):
                ...
                if compare_image(original, corrected_sync):
                    successful_convergences_sync += 1
                    steps_data.append(steps_sync)
                    print(
                        f"Probability : {prob * 0.1} Run {run} Image {imgn} successfully converges in \
                                        {steps_sync} Synchronous steps")
                if compare_image(original, corrected_async):
                    successful_convergences_async += 1
                    steps_data.append(steps_async)
                    print(
                        f"Probability : {prob * 0.1} Run {run} Image {imgn} successfully converges in \
                                        {steps_async} Asynchronous steps")

        convergence_rates_flip_sync.append(successful_convergences_sync / (20 * len(dataset)))
        convergence_rates_flip_async.append(successful_convergences_async / (20 * len(dataset)))
        steps_histogram_data[prob] = steps_data

    # get data for successful converges when the images are cropped
    # for each image
    sync_successful_convergences = [0, 0, 0, 0]
    async_successful_convergences = [0, 0, 0, 0]
    for _ in range(20):
        ...
        for img, original in zip(corrupted_images, dataset):
            ...
            for i in range(4):
                if corrected_sync[i] == original[i]:
                    sync_successful_convergences[i] += 1
                if corrected_async[i] == original[i]:
                    async_successful_convergences[i] += 1

    # plot data for converges when images are cropped
    plt.figure(figsize=(8, 5))
    plt.bar([img_ for img_ in ["circle", "cross", "plus", "square"]],
            [i/40 for i in sync_successful_convergences],
            color='red')
    plt.xlabel("Correct Image")
    plt.ylabel("Converged Runs (Synchronous Updates)")
    plt.title("Convergence for cropped images vs Correct Image (Synchronous Updates)")
    plt.show()

    plt.figure(figsize=(8, 5))
    plt.bar([img_ for img_ in ["circle", "cross", "plus", "square"]],
            [i / 40 for i in async_successful_convergences],
            color='red')
    plt.xlabel("Correct Image")
    plt.ylabel("Converged Runs (Asynchronous Updates)")
    plt.title("Convergence for cropped images vs Correct Image (Asynchronous Updates)")
    plt.show()

    # plot data for converges when pixels are randomly flipped
    plt.figure(figsize=(8, 5))
    plt.bar([("0." + str(p)) for p in probabilities], convergence_rates_flip_sync, color='blue')
    plt.xlabel("Flip Probability")
    plt.ylabel("Fraction of Correctly Converged Runs (Synchronous Updates)")
    plt.title("Convergence Rate vs Flip Probability (Synchronous Updates)")
    plt.show()

    plt.figure(figsize=(8, 5))
    plt.bar([("0." + str(p)) for p in probabilities], convergence_rates_flip_async, color='blue')
    plt.xlabel("Flip Probability")
    plt.ylabel("Fraction of Correctly Converged Runs (Asynchronous Updates)")
    plt.title("Convergence Rate vs Flip Probability (Asynchronous Updates)")
    plt.show()
